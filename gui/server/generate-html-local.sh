...
rm -rf cgi-local
mkdir cgi-local
for i in `ls -d ./* | grep -v '\.pyc' | grep -v 'cgi-local' | grep -v 'serverconfig' | grep -v 'attractserver' | grep -v 'upload.py'`; do
 cd cgi-local
 ln -s ../$i
 cd ..
done
...
for i in `ls -d ./* | grep 'attractserver'`; do
 cd cgi-local
 ln ../$i #hard-link! editing attractserver-XXX.py in an external editor will NOT update the other one!
 cd ..
done
cd cgi-local
ln ../upload.py
...
mkdir html-local
for i in `ls -ad html/* | \
grep -v 'full.html' | grep -v 'easy.html' | grep -v 'standard.html' | grep -v 'cryo.html' | grep -v 'upload.html' | grep -v 'peptide.html' | \
