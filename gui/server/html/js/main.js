$(document).ready(function() {
    ...
    renumberFormBlocks = function($root) {
        ...
        getCloneBlocks($root).each(function() {
            ...
            // If ID equals 0 it is the root, do nothing.
    ...
    cloneBlock = function($blockID, $max) {
        ...
        if (count + 1 <= $max) {
            ...
            // Remove all highlights from the title (remove embedded resource markings)
            $clone.find("em.inner").replaceWith( "<h2>New heading</h2>" );
    ...
    // Hide all 'remove' buttons from form blocks with pattern block-*-0
    $('div[id^=block-]').each(function() {
