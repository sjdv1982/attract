class IntegerList(IntegerArray):
  def __arrayvalidate__(self):    
    for v in range(len(self)): 
      try:
        Integer(self[v])
      except:
        raise ValueError("Invalid number '%s'. Please specify as comma-separated numbers, e.g. 1,2,3" % self[v])
...
spyder.__types__["IntegerList"] = IntegerList
spyder.core.arrayfunc("IntegerList", globals())
