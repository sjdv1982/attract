...
"""
    ...
    ret += "if [ ! -s $mask1 ]; then\n"
    ret += " python $ATTRACTTOOLS/em/situs2mask.py $mapfile $mapmask_threshold %d %d $mask1 ${mask1%%%%.*}.sit\n" % (m.mapmask1_voxelsize, m.mapmask1_dimension)
    ret += " python $ATTRACTTOOLS/em/situs2mask.py $mapfile $mapmask_threshold %d %d $mask2 ${mask2%%%%.*}.sit\n" % (m.mapmask2_voxelsize, m.mapmask2_dimension)
    ret += "fi\n"
    ...
    noclone = "\'--ori %.6f --trans %.6f\'" % (m.clone_rot[1]*fr, m.clone_center[1]*fc)
    ret += "noclone=%s\n" % noclone
    clone = "\'--ori %.6f --trans %.6f --clone 10 --keepfirst\'" % (m.clone_rot[1]*fr, m.clone_center[1]*fc)
    ret += "clone=%s\n" % clone
    fast = ""
