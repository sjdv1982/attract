...
c     Local variables      
      ...
      integer dseed,i,ii,j,jj,k,kk,itr,nfun
...
c        write(*,*)'before',phi(i),ssi(i),rot(i)
        call crand(dseed,5,rr)
        dseed = int(10000*rr(5))
        rr(1)=rr(1)-0.5d0
        rr(2)=rr(2)-0.5d0
        rr(3)=rr(3)-0.5d0
...
c make a translation of the ligand center
      ...
      ya(i)=xaa(ii+2)+scalecenter*(0.5d0-rr(ii+2))
      za(i)=xaa(ii+3)+scalecenter*(0.5d0-rr(ii+3))
c     write(*,*)'new ii,c,xa ya za',i,ii,c,xa(i),ya(i),za(i)
 1220 continue
      endif
      do 183 i=1,3
...
c      write(*,*)'exp(bol)',enew,gesa,enew-gesa,bol
      call crand(dseed,2,rr)
      dseed = int(10000*rr(2))
      if(bol.gt.rr(1)) then
