 ...
 * (c) Willy Wriggers, Pablo Chacon, and Paul Boyle 2001-2009         *
 **********************************************************************
 *                                                                    *
...
void interpolate_map (double **outmap, unsigned *out_extx, unsigned *out_exty, unsigned *out_extz, 
  ...
  isx_out = ceil (in_origx/out_widthx);
  isy_out = ceil (in_origy/out_widthy);
  isz_out = ceil (in_origz/out_widthz);

  /* output end index rel. to coordinate system origin, asserting that outmap is fully embedded in inmap */
  iex_out = floor ((in_origx+in_widthx*(in_extx-1))/out_widthx); 
  iey_out = floor ((in_origy+in_widthy*(in_exty-1))/out_widthy); 
  iez_out = floor ((in_origz+in_widthz*(in_extz-1))/out_widthz); 
...
void project_map_lattice (double **outmap, unsigned ref_extx, unsigned ref_exty, unsigned ref_extz, 
  ...
  for (indz=0;indz<ref_extz;indz++)
	...
	if (x0>=0 && x1<in_extx && y0>=0 && y1<in_exty && z0>=0 && z1<in_extz) {
	  a = gx-x0;
	  b = gy-y0;
	  c = gz-z0;	  
...
/*====================================================================*/
/* efficiently computes three map density distribution parameters */
void calc_map_info_short (double *phi, unsigned long nvox, double *maxdens, double *mindens, double *ave) {
  unsigned long m;
...
int print_histogram(unsigned *extx, unsigned *exty, unsigned *extz, double **phi, int nbins) {
  ...
  printf ("lib_vwk> Min. / max. density values: %f / %f\n", mindensity, maxdensity);
  ...
  if (maxdensity>mindensity) {  
    ...
    for (count=0;count<nvox;count++) {
      current = floor(0.5 + ( *(pphi+count) - mindensity) * scale);
      ++his[current];
    }
    
    /* determine two fullest bins and normalize */
    currp = 0;
    currnp = 0;
    nextpeak = 0;
    peak = 0;
    ...
    if (his[nextpeak]>0) { 
      for (j=0;j<nbins;j++) {
	phis[j] = floor((BARL*his[j]/his[nextpeak])+0.5);
	if (phis[j]>BARL) phis[j] = BARL+1;
      }
    } else { 
      if (his[peak]>0) { 
	for (j=0;j<nbins;j++) {
	  phis[j] = floor((BARL*his[j]/his[peak])+0.5);
	  if (phis[j]>BARL) phis[j] = BARL+1;
	}
    ...
    for (j=0;j<nbins;j++) {
      ...
      if (his[peak]>0) {
	if (his[j]>0) {
	  if (phis[j] <= BARL) for (i=(floor(log10(his[j]))-floor(log10(his[peak]))-BARL-3+phis[j]);i<0;++i) {
	    if ((j%2)||(i%4)) printf (" ");
	    else printf (".");
	  }
	} else {
	  for (i=-floor(log10(his[peak]))-BARL-3+phis[j];i<0;++i) {
	    if ((j%2)||(i%4)) printf (" ");
	    else printf (".");
	  }
	}
