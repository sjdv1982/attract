FROM ubuntu:12.04
RUN apt-get update && apt-get install -y wget g++ gfortran python-numpy
RUN apt-get upgrade -y
RUN apt-get install -y python-scipy pdb2pqr make python-pip --fix-missing
...
RUN pip install  --index-url=https://pypi.python.org/simple/ peptidebuilder numpy scipy
ADD . /attract
