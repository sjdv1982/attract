...
from lib.read_struc import read_struc
...
coor_chunksize = 20000
grid_chunksize = 10000 #for 20x20x20 grid
...
if not args.mc or not args.ghost:
  cmd = "python $ATTRACTDIR/../protocols/attract.py " + " ".join(sys.argv[1:])
...
pivots, eulers, positions = read_struc(datfile, coms) 
emdata, gridspacing, origin = read_situs(emfile)
s = emdata.shape[0] 
assert s in (20, 40), emdata.shape
if s == 40: 
  grid_chunksize = 2000
...
def energy(eulers, positions):
  overlaps = c_overlap(eulers, positions, templates, weights, tuple(origin), \
