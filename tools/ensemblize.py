...
if len(sys.argv) not in  (5,6):
  print("Usage: ensemblize <DOF file> <ensemble size> <molecule ID> <mode='all'/'random'> [random seed]", file=sys.stderr)
  sys.exit()
...
for s in structures: 
  if mode == "all": ensemble = list(range(1,ens+1))
  elif mode == "random": ensemble = [random.choice(list(range(1,ens+1)))]
  for e in ensemble:
    stnr += 1
    print("#"+str(stnr))
    l1,l2 = s
